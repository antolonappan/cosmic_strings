...
def build_extension(force: bool = False, verbose: bool = True) -> Path:
    """Build the Fortran shared library in this folder."""
...
def compute_cl(
    ...
    *,
    ...
    Parameters intentionally mirror ``strings.compute_cl`` where practical.
    ``progress`` and ``max_workers`` are accepted so the same notebook code can
    switch between backends. The Fortran kernel itself runs serially.
    """
